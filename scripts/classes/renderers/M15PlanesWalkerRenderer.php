...
////////////////////////////////////////////////////////////////////////
class M15PlanesWalkerRenderer extends CardRenderer {
	...
	public function render () {
		...
		if ($card->isArtefact()) {
			$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/Art.png");
		} else if ($useMulticolorFrame || $card->isDualManaCost()) {
			// Multicolor frame.
			if($settings['card.multicolor.gold.frame'])
				$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/Gld$costColors.png");
			else
				$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/$costColors.png");
			if (!$bgImage) error("Background image not found for color: $costColors");
		} else {
			// Mono color frame.
			$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/" . $card->color . '.png');
		...
		if ($card->artist) {
			...
			} else if (in_array($card->set, $promoset)) {
			...
			if (($promoSetText = $this->getPromoSetText($card->set)) && in_array($card->set, $promoset)) {
	...
	private function getPromoSetText ($set) {
		if (!M15PlanesWalkerRenderer::$titleToPromoSetText) M15PlanesWalkerRenderer::$titleToPromoSetText = csvToArray('data/setToPromoText.csv');
		return @M15PlanesWalkerRenderer::$titleToPromoSetText[(string)strtolower($set)];
	}
	
	private function getPromoCardText ($title) {
		if (!M15PlanesWalkerRenderer::$titleToPromoCardText) M15PlanesWalkerRenderer::$titleToPromoCardText = csvToArray('data/titleToPromoText.csv');
		return @M15PlanesWalkerRenderer::$titleToPromoCardText[(string)strtolower($title)];
	}
	...
	public function getSettings () {
		...
		$settings = $rendererSettings['config/config-m15planeswalker.txt'];
		$frameDir = $this->getFrameDir($this->card->title, $this->card->set, $settings);
		$settings = array_merge($settings, $rendererSections['config/config-m15planeswalker.txt']['fonts - ' . $frameDir]);
		$settings = array_merge($settings, $rendererSections['config/config-m15planeswalker.txt']['layout - ' . $frameDir]);
		return $settings;
	}

	private function getFrameDir ($title, $set, $settings) {
		if (!M15PlanesWalkerRenderer::$titleToTransform) M15PlanesWalkerRenderer::$titleToTransform = csvToArray('data/titleToTransform.csv');
		$frameDir = 'transform-' . @M15PlanesWalkerRenderer::$titleToTransform[(string)strtolower($title)];
