...
////////////////////////////////////////////////////////////////////////
class M15PlanesWalker2Renderer extends CardRenderer {
	...
	public function render () {
		...
		if ($card->isArtefact()) {
			$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/Art2.png");
		} else if ($useMulticolorFrame || $card->isDualManaCost()) {
			// Multicolor frame.
			if($settings['card.multicolor.gold.frame'])
				$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/Gld" . $costColors . "2.png");
			else
				$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/" . $costColors . "2.png");
			if (!$bgImage) error("Background image not found for color: $costColors");
		} else {
			// Mono color frame.
			$bgImage = @imagecreatefrompng("images/m15/planeswalker/$frameDir/cards/" . $card->color . '2.png');
	...
	public function getSettings () {
		...
		$settings = $rendererSettings['config/config-m15planeswalker2.txt'];
		$frameDir = $this->getFrameDir($this->card->title, $this->card->set, $settings);
		$settings = array_merge($settings, $rendererSections['config/config-m15planeswalker2.txt']['fonts - ' . $frameDir]);
		$settings = array_merge($settings, $rendererSections['config/config-m15planeswalker2.txt']['layout - ' . $frameDir]);
		return $settings;
	}

	private function getFrameDir ($title, $set, $settings) {
		if (!M15PlanesWalker2Renderer::$titleToTransform) M15PlanesWalker2Renderer::$titleToTransform = csvToArray('data/titleToTransform.csv');
		$frameDir = 'transform-' . @M15PlanesWalker2Renderer::$titleToTransform[(string)strtolower($title)];
